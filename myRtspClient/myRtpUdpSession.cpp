    ...
    *size = 0;
	...
	do {
		...
		if (!GotoFirstSourceWithData()) {
			...
			//fprintf(stderr, "GotoFirstSourceWithData return Not Null \r\n");
			EndDataAccess();
			usleep(USLEEP_UNIT);
		...
		{
			EndDataAccess();
			//fprintf(stderr, "GetNextPacket return NULL \r\n");
			usleep(USLEEP_UNIT);
			continue;
			// return NULL;
		}
		...
		*size = PacketSize;
		...
		static int temp = ntohs(pack->GetSequenceNumber());//
		int SequenceNumber = ntohs(pack->GetSequenceNumber());
		if (temp + 256 != SequenceNumber)
		{
			//fprintf(stdout, "miss: last %d curr %d \r\n", temp, ntohs(pack->GetSequenceNumber()));
		}
